...
#endif
*/
/// my modification below, replacing text in quotes above
#define ext
#define extend
extern "C" {
	/// 
...
#define DIO64_ERR_NOTENOUGHTRANS							-26
	...
	ext int DLLEXPORT DIO64_Open(WORD board, WORD baseio);
	ext int DLLEXPORT DIO64_Mode(WORD board, WORD mode);
	ext int DLLEXPORT DIO64_Load(WORD board, char *rbfFile, int intputHint, int outputHint);
	ext int DLLEXPORT DIO64_Close(WORD board);

	ext int DLLEXPORT DIO64_In_Start(WORD board,
		DWORD ticks,
		WORD *mask,
		WORD maskLength,
		WORD flags,
	...
	ext int DLLEXPORT DIO64_In_Status(WORD board, DWORD *scansAvail, DIO64STAT *status);
	ext int DLLEXPORT DIO64_In_Read(WORD board, WORD *buffer, DWORD scansToRead, DIO64STAT *status);

	ext int DLLEXPORT DIO64_In_Stop(WORD board);

	ext int DLLEXPORT DIO64_Out_ForceOutput(WORD board, WORD *buffer, DWORD mask);
	ext int DLLEXPORT DIO64_Out_GetInput(WORD board, WORD *buffer);
	ext int DLLEXPORT DIO64_Out_Config(WORD board,
		DWORD ticks,
		WORD *mask,
		WORD maskLength,
		WORD flags,
	...
	ext int DLLEXPORT DIO64_Out_Start(WORD board);

	ext int DLLEXPORT DIO64_Out_Status(
		WORD board,
		DWORD *scansAvail,
		DIO64STAT *status);

	ext int DLLEXPORT DIO64_Out_Write(WORD board, WORD *buffer, DWORD bufsize, DIO64STAT *status);
	ext int DLLEXPORT DIO64_Out_Stop(WORD board);

	ext int DLLEXPORT DIO64_SetAttr(WORD board, DWORD attrID, DWORD value);
	ext int DLLEXPORT DIO64_GetAttr(WORD board, DWORD *attrID, DWORD *value);
	/*
	#ifdef __cplusplus
	#define extend }
	#endif
	*/
	/// my modification (the single right brace) below, replacing commented text above
}
///
#endif
